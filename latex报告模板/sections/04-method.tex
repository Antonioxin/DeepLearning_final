\section{融合 VGGT 与 TRELLIS 的三维场景编辑方法}

\subsection{问题定义}

给定同一真实场景的 $N$ 张无标定 RGB 图像
$\mathcal{I}=\{I_i\}_{i=1}^{N}$，以及描述待生成对象的文本或参考图像条件 $c$，
本文目标是在不重新训练 VGGT 与 TRELLIS 的前提下，生成编辑后的三维场景
...
整体框架如图 \ref{fig:pipeline} 所示，由统一场景重建、语言定位、条件生成、空间注册、
...
TRELLIS 以文本或参考图像为条件生成 mesh 或 3DGS 资产。系统再利用场景支撑面、目标包围盒
...
\begin{enumerate}[label=\arabic*.]
  ...
  系统优先利用其他视图已观测到的背景高斯回填；仍有空洞时，再使用 TRELLIS 局部重采样或多视图纹理补全。
  该分支属于方法设计，不计入本文已完成的添加对象复现结果。
\end{enumerate}
...
图 \ref{fig:mllm-language-edit} 展示了这一语言驱动编辑路径的典型交互过程。用户首先要求删除机甲手臂，系统通过 MLLM 解析指令并定位局部部件；随后连续提出“装上武器双肩”和“换上坦克履带底座”，每轮只更新当前目标区域，其余结构作为保护区域继承。该图用于说明“指令解析--局部定位--编辑算子--状态继承”的方法设计，不作为本文教室场景实验的定量结果。
...
该方法的核心是把 VGGT 从一次性的重建前端扩展为贯穿编辑前后的几何接口：编辑前，
它建立统一坐标与目标位置；编辑后，它判断多视图结果是否仍符合真实三维数据先验。
TRELLIS 则专注于补充真实观测中不存在的新内容。二者与 MLLM/SAM2 定位、显式相似变换、
局部可微渲染和闭环评分共同形成“重建--定位--生成--融合--编辑--验证”链路，而不是两项独立模型的简单串联。
...
\begin{enumerate}[label=\arabic*.]
  ...
  便于导入 Blender、Unity 等工具继续使用\cite{trellis,trellis2}。当前代码已提供 TRELLIS 1 的多格式 decoder 与 GLB/PLY 导出函数，但本地尚缺带条件和随机种子记录的 TRELLIS 生成产物；
