...
本地修订为 \texttt{442aa1e}。两个子工程均保留了官方推理、可视化与训练代码。

需要强调的是，当前代码中没有直接同时导入 \texttt{vggt} 与
\texttt{trellis} 的融合入口，也没有支撑面估计、资产相似变换、SAM2/MLLM
定位、局部可微融合或 VGGT 闭环评分的可执行实现。因此，第 4 章给出的是基于
现有模型接口可以实现的系统方法，而本章只把已在源码中出现的功能记为“已实现”。

\begin{table}[H]
  \centering
  \caption{本地代码中已实现模块及其输出}
  \label{tab:code-scope}
  \small
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{2.5cm} p{4.2cm} X}
    \toprule
    \textbf{模块} & \textbf{主要入口} & \textbf{代码可验证功能} \\
    \midrule
    VGGT 前馈 & \path{demo_gradio.py}\\\path{demo_viser.py} &
    输出相机编码、深度/置信度、世界 pointmap/置信度；可选跟踪头输出 track、visibility 和 confidence。\\
    VGGT 导出 & \path{visual_util.py}\\\path{demo_colmap.py} &
    将 pointmap 或“深度+相机”反投影为彩色点云，进行置信度百分位过滤，导出 GLB；另可导出 COLMAP 相机、图像和点云文件。\\
    TRELLIS 生成 & \path{example.py}\\\path{example_text.py} &
    从单图或文本条件先采样 sparse structure，再采样 SLat，同一 SLat 可解码为 mesh、3D Gaussian 和 Radiance Field。\\
    TRELLIS 扩展 & \path{example_multi_image.py}\\\path{example_variant.py} &
    支持训练自由的多图条件和基于已有 mesh 体素结构的文本变体生成。\\
    融合编辑 & 尚无统一入口 &
    第 4 章的注册、局部融合、语言定位与几何闭环尚需实现，不应与两个官方演示程序混为一谈。\\
    \bottomrule
  \end{tabularx}
\end{table}
...
故本文不对这些字段作猜测。表 \ref{tab:environment} 严格列出源码或项目配置能够确认的依赖，
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{2.8cm} p{3.0cm} X}
    ...
    其中 1 个 primitive 使用 glTF mode 0（POINTS）并含 1,000,000 个彩色点；443 个 primitive 使用 mode 1（LINES），且无纹理图像。这与 VGGT Gradio 导出的“点云+相机线框”结构一致，不是 TRELLIS 纹理 mesh 的证据。\\
    \bottomrule
  \end{tabularx}
\end{table}
...
“已归档的 Gaussian 产物”，而不宣称当前两个子仓库可以端到端重现它。
...
为保证下一阶段可复现，融合模块应至少保存：VGGT 预测的原始 \texttt{npz}，
